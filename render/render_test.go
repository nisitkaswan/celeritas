...
var pageData = []struct {
	...
	{"go_page_no_template", "go", "no-file", true, "no error rendering non-existing template, when one is expected"},
	{"jet_page", "jet", "home", false, "error rendering jet template\""},
	{"jet_page_no_template", "jet", "no-file", true, "no error rendering non-existing template, when one is expected"},
	{"invalid_render_engine", "foo", "home", true, "no error rendering non-existing template, when one is expected"},
}
